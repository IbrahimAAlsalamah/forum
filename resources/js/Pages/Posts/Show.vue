<script setup>
  ...
  const deleteComment = async (comment_id) => {
      ...
      router.delete(route('comments.destroy', {
          comment: comment_id, page: props.comments.meta.current_page
      }), {preserveScroll: true});
  };
      ...
      <Container>
         ...
         <PageHeading class="mt-2">{{ post.title }}</PageHeading>
          <div class="mt-1 space-x-3 flex items-center">
              <span class="text-pink-600 font-bold text-sm">{{ post.likes_count }} likes</span>
              ...
              <span class="text-gray-500 text-sm">{{ formattedDate }} ago by {{ post.user.name }}</span>
          </div>
        ...
        <article class="mt-4">
          ...
          <div class="mt-6">
              ...
              <ul class="divide-y mt-2">
                  <li v-for="comment in comments.data" :key="comment.id" class="block px-2 pt-4 pb-1">
                      <Comment @edit="editComment" @delete="deleteComment(comment.id)" :comment="comment" />
                  </li>
              </ul>
